...
/*void getExternalWrench (Vector6d& external) {
	...
	if((krang->fts[LEFT]->lastExternal.topLeftCorner<3,1>().norm() > 7) || 
		...
		Vector6d temp (krang->fts[LEFT]->lastExternal);
		temp(0) = 0.0;		// ignoe x axis forces
		computeWheelWrench(temp, *robot, leftWheelWrench, true);
	}
...
/*void computeBalAngleRef(const Vector3d& com, double externalTorque, double& refImu) {
	...
	double com_x = (externalTorque + kasdf * last_u_x) / (totalMass * 9.81);
...
void init() {
	...
	int hwMode = Krang::Hardware::MODE_AMC | Krang::Hardware::MODE_LARM | 
		Krang::Hardware::MODE_RARM | Krang::Hardware::MODE_TORSO | Krang::Hardware::MODE_WAIST| Krang::Hardware::MODE_GRIPPERS_SCH;
