...
public:
    ...
    : dt_(time_step), H_(time_steps), iter_(iterations), logger_(logger), verbose_(verbose),
      ...
      boxqp_(qp_options, logger_), warm_start_(false)
    {
                                   ...
                                   const Eigen::Ref<const Control> &u_max = Control::Constant(std::numeric_limits<Scalar>::infinity()))
    {
        ...
        while(current_iteration < iter_)
        {
            ...
            for(int k = 0; k < H_; ++k)
            {
                ...
                d2L_.at(k) = run_cost.d2c(x_.col(k), u_.col(k));
                dL_.col(k) = run_cost.dc(x_.col(k), u_.col(k));
                L_(k) = run_cost.c(x_.col(k), u_.col(k));
            }
            ...
            for(int k = H_ - 2; k >= 0; --k)
            {
                ...
                if(Eigen::isfinite(u_max.array().abs()).any() || Eigen::isfinite(u_min.array().abs()).any())
                {
                    ...
                    {
                        no_qp = true;
                        if(verbose_) logger_->warning("DDP: BoxQP failed, clamp controls instead\n");
                    }
                    else
                    {
                        lk_.col(k) = qp_result.solution;
                        Lk_.at(k) = Eigen::Matrix<Scalar, DynamicsT::ControlSize, DynamicsT::StateSize>::Zero();
                        if(qp_result.free_indices.any())
                        {
                            Eigen::Matrix<Scalar, Eigen::Dynamic, DynamicsT::StateSize> qux_free(qp_result.free_indices.count(), static_cast<int>(DynamicsT::StateSize));
                            int j = 0;
                            ...
                            Eigen::Matrix<Scalar, Eigen::Dynamic, DynamicsT::StateSize> Lfree = qp_result.free_chol.solve(-qux_free);
                            j = 0;
                            for(int i = 0; i < DynamicsT::ControlSize; ++i)
                            {
                                if(qp_result.free_indices(i))
                ...
                if(no_qp)
                {
                    Eigen::LDLT<Eigen::Ref<Eigen::Matrix<Scalar, DynamicsT::ControlSize, DynamicsT::ControlSize>>> ldlt(quu_);
            ...
            while(!accept)
            {
                ...
                for(std::size_t k = 0; k < H_ - 1; ++k)
                {
                    ...
                    cost_(current_iteration, k) = run_cost.c(xnew.col(k), unew.col(k)) * dt_;
                }
